    ...
    <div className="min-h-screen bg-black-100 text-gray-100 py-36 md:px-8 lg:px-12">
      <div className="container mx-auto">
        <header className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-blue-400">Quiz Results</h1>
          <p className="text-lg sm:text-xl mt-1 font-semibold">Your score: {score} / {questionsData.length}</p>
        </header>
        <div className="space-y-6 sm:space-y-8">
          {questionsData.map((question: Question) => (
            <div key={question.id} className="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-md">
              <h2 className="text-xl sm:text-2xl font-semibold text-purple mb-2">
                {`Q${question.id}. ${question.text}`}
              </h2>
              <SyntaxHighlighter language="javascript" style={dracula} customStyle={{ borderRadius: '8px' }}>
                {question.code}
              </SyntaxHighlighter>
              <div className="mt-4 space-y-2">
                ...
                <p className={`text-base sm:text-lg font-semibold ${selectedOptions?.[question.id] ===
                question.options.find((option) => option.isCorrect)?.text ? "text-green-400" : "text-red-400"}`}>
